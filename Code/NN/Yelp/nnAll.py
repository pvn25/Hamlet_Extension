...
y_vec = []
label = 1
with open('YRtrain_new.csv','r') as f1:
	...
	for row in reading1:
		...
		if row[label] =='0':
			...
			y_vec.append(1)
with open('YRtest_new.csv','r') as f1:
	reading1 = csv.reader(f1,delimiter = ',')
	i = 0
	for row in reading1:
		if i == 0:
...
with open('SJCYRtraintest.csv','r') as f:
...
#############################################################################################
y_vec_hold = []
with open('YRhold_new.csv','r') as f1:
	reading1 = csv.reader(f1,delimiter = ',')
	i = 0
	for row in reading1:
		if i == 0:
...
with open('SJCYRhold.csv','r') as f:
    reading = csv.reader(f,delimiter=',')
    i = 0
    max_row = 0
    max_col = 0
...
sparse_mat_train = sparse_mat[:107940,:]
y_vec_train = y_vec[:107940]
sparse_mat_test = sparse_mat[107940:,:]
y_vec_test = y_vec[107940:]
start = time.time()
