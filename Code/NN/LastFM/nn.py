...
y_vec = []
label = 2
with open('LFsub2train_new.csv','r') as f1:
...
with open('LFsub2test_new.csv','r') as f1:
	reading1 = csv.reader(f1,delimiter = ',')
	i = 0
	for row in reading1:
		if i == 0:
...
with open('LFCAtraintest.csv','r') as f:
...
#############################################################################################
y_vec_hold = []
with open('LFsub2hold_new.csv','r') as f1:
	reading1 = csv.reader(f1,delimiter = ',')
	i = 0
	for row in reading1:
		if i == 0:
...
# sparse_mat_train, sparse_mat_test, y_vec_train, y_vec_test = train_test_split(sparse_mat, y_vec, test_size=0.001, random_state=seed)
sparse_mat_train = sparse_mat[:171874,:]
y_vec_train = y_vec[:171874]
sparse_mat_test = sparse_mat[171874:,:]
y_vec_test = y_vec[171874:]
start = time.time()
#  W_regularizer=l1(0.01),
# glorot_normal
reg = 0.001
model = Sequential()
model.add(Dense(256, input_dim=cols,  W_regularizer = l1(reg),init='glorot_normal', activation='relu'))
# model.add(Dropout(0.5))
# model.add(BatchNormalization())
model.add(Dense(64, init='glorot_normal',  W_regularizer = l1(reg), activation='relu'))
# model.add(Dropout(0.5))
# model.add(BatchNormalization())
# model.add(Dense(64, init='glorot_normal',  W_regularizer = l2(reg), activation='relu'))
# model.add(Dropout(0.5))
# model.add(BatchNormalization())
model.add(Dense(1, init='glorot_normal',  W_regularizer = l1(reg), activation='sigmoid'))
