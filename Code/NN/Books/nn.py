...
label = 2
y_vec = []
with open('BCtrain_new.csv','r') as f1:
	...
	for row in reading1:
		...
		if row[label] == "'0'":
			...
			y_vec.append(1)
# print('fsd')
with open('BCtest_new.csv','r') as f1:
	reading1 = csv.reader(f1,delimiter = ',')
	i = 0
	for row in reading1:
		if i == 0:
...
with open('SCABCtraintest2.csv','r') as f:
...
#############################################################################################
y_vec_hold = []
with open('BChold_new.csv','r') as f1:
	reading1 = csv.reader(f1,delimiter = ',')
	i = 0
	for row in reading1:
		if i == 0:
...
with open('SCABChold2.csv','r') as f:
    reading = csv.reader(f,delimiter=',')
    i = 0
    max_row = 0
    max_col = 0
...
# sparse_mat_train, sparse_mat_test, y_vec_train, y_vec_test = train_test_split(sparse_mat, y_vec, test_size=0.33)
sparse_mat_train = sparse_mat[:126560,:]
y_vec_train = y_vec[:126560]
sparse_mat_test = sparse_mat[126560:,:]
y_vec_test = y_vec[126560:]
start = time.time()
