...
{
pt = proc.time(); 
tunn <- svm(position ~., data = WTtrain,  cost = ms[i], gamma = cpv[j],kernel= "radial", cachesize = 60000)  
...
j <- 1
  ...
  pt = proc.time(); 
  tunn <- svm(position ~., data = WTtrain,  cost = best_ms, gamma = best_cpv,kernel= "radial", cachesize = 60000)  
