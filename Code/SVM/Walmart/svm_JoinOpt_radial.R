...
{
pt = proc.time(); 
tunn <- svm(weekly_sales ~., data = WTtrain,  cost = ms[i], gamma = cpv[j],kernel= "radial", cachesize = 60000)  
...
j <- 1
  ...
  pt = proc.time(); 
  tunn <- svm(weekly_sales ~., data = WTtrain, cost = best_ms, gamma =best_cpv ,kernel= "radial", cachesize = 60000)
