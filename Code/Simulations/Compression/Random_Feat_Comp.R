...
print(ho)
while(i < rows){
	set.seed(OFtrain$airlineid[i])
	OFtrain$airlineid[i] <- sample(1:tr,1,replace = T)
	set.seed(OFtrain$sairportid[i])
	OFtrain$sairportid[i] <- sample(1:te,1,replace = T)
	set.seed(OFtrain$dairportid[i])
	OFtrain$dairportid[i] <- sample(1:ho,1,replace = T)
	if(i < rows/2){
	set.seed(OFtest$airlineid[i])
	OFtest$airlineid[i] <- sample(1:tr,1,replace = T)
	set.seed(OFtest$sairportid[i])
	OFtest$sairportid[i] <- sample(1:te,1,replace = T)
	set.seed(OFtest$dairportid[i])
	OFtest$dairportid[i] <- sample(1:ho,1,replace = T)

	set.seed(OFhold$airlineid[i])
	OFhold$airlineid[i] <-sample(1:tr,1,replace = T)
	set.seed(OFhold$sairportid[i])
	OFhold$sairportid[i] <- sample(1:te,1,replace = T)
	set.seed(OFhold$dairportid[i])
	OFhold$dairportid[i] <- sample(1:ho,1,replace = T)
	}
	i = i + 1
}
