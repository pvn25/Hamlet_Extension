...
YRtrain=read.csv("YRtrain_numeric.csv");
YRtest=read.csv("YRtest_numeric.csv");
YRhold=read.csv("YRhold_numeric.csv");
YRtrain=YRtrain[,-1] #get rid of reviewid
YRtest=YRtest[,-1]
YRhold=YRhold[,-1]# print(str(OFtrain))
# print(str(OFtest))
# print(str(OFhold))
#print(levels(OFtrain$airlineid))
rows = nrow(YRtrain)
i = 1
x = 5000
tr = x
...
print(te)
print(ho)
print(YRtest$userid[1])
while(i < rows){
	set.seed(as.integer(YRtrain$userid[i]))
	YRtrain$userid[i] <- sample(1:tr,1,replace = T)
	set.seed(as.integer(YRtrain$businessid[i]))
	YRtrain$businessid[i] <- sample(1:te,1,replace = T)
	if(i < (rows/2) -1){
	set.seed(as.integer(YRtest$userid[i]))
	YRtest$userid[i] <- sample(1:tr,1,replace = T)
	set.seed(as.integer(YRtest$businessid[i]))
	YRtest$businessid[i] <- sample(1:te,1,replace = T)
	
	set.seed(as.integer(YRhold$userid[i]))
	YRhold$userid[i] <-sample(1:tr,1,replace = T)
	set.seed(as.integer(YRhold$businessid[i]))
	YRhold$businessid[i] <- sample(1:te,1,replace = T)
	}
	i = i + 1
}
