...
OFtrain = OFtrain[,c(-2,-3,-4)]
OFtest = OFtest[,c(-2,-3,-4)]
OFhold = OFhold[,c(-2,-3,-4)]
# sink("jallinfonofk_variablesplit.txt")
if(TRUE){
ms = c(1,10,100,1000)
...
{
pt = proc.time(); 
fit <- rpart(codeshare~., data=OFtrain, method = "class",control=rpart.control(minsplit=ms[i], cp=cpv[j]),parms = list(split = 'information'))
...
#sink(jallinfonofk.txt)
#print(tm)
# print(str(OFtrain))
pt = proc.time(); 
print(best_ms)
print(best_cpv)
fit <- rpart(codeshare~., data=OFtrain, method = "class",control=rpart.control(minsplit=best_ms ,cp=best_cpv),parms = list(split = 'information'))
#fit <- prune(fitt,cp = 0.001)
summary(fit)
