...
#cpv = c(0.0001,0.00001,0.000001,0.00000001,0.00000001,0.000001,0.000001,0.000001)
...
{
pt = proc.time(); 
fit <- rpart(stars~., data=YRtrain, method = "class",control=rpart.control(minsplit=ms[i], cp=cpv[j]),parms = list(split = 'information'))
...
print(best_ms)
print(best_cpv)
fit <- rpart(stars~., data=YRtrain, method = "class",control=rpart.control(minsplit=best_ms, cp=best_cpv),parms = list(split = 'information'))
