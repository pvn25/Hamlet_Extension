...
<<<<<<< HEAD
ratio, we used \red{TODO} CORElearn library.} and ``e1071'' for the SVM. We use the validation set to perform hyper-parameter tuning using a standard grid search as follows:
...
must exist in a node for a split to be attempted. Any split that does not improve the fit by a factor of \textit{cp} is pruned off. The grid is set as follows: \textit{minsplit} $\in \{ 1, 10, 100, 10^3\}$ and \textit{cp} $\in \{10^{-4}, 10^{-3}, 0.01, 0.1, 0\}$   
=======
...
\centering
<<<<<<< HEAD
\includegraphics[width=0.99\linewidth]{onexr.pdf}
=======
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth,height=\textheight,keepaspectratio]{dom_comp.pdf}
\caption{Domain Compression applied on (A) Flights Dataset (B) Yelp Dataset}
...
<<<<<<< HEAD
five runs. Figure~\ref{Figure:Compression} presents the results. 
=======
