...
\title{Discarding Data Tables Without Even Looking At Them}
...
Let \textbf{T} denote the output of the projected equi-join query that constructs the full training dataset by \textit{concatenating} the features from all base tables: 
$\textbf{T} \leftarrow \pi(\textbf{R} \bowtie_{RID=FK} \textbf{S})$. In general, its schema is 
...
\section{KFK Joins and Non-linear Classifiers}
...
The above optimization problem is not convex and hard to optimize. Thus, apart from a random hashing-based compression baseline, we evaluate two new heuristics:
\begin{packeditems}
\item \textit{SortBased}: This is a greedy approach in which we sort $z \in \mathcal{D}_{FK}$ on $H(Y|FK=z)$, compute the differences among adjacent pairs of values, 
and pick the boundaries corresponding to the top $l$ differences. This gives us an $l$-partition of $\mathcal{D}_{FK}$. The intuition is that by grouping $FK$ 
values that have comparable conditional entropy values, $H(Y|f(FK))$ is unlikely to increase much compared to $H(Y|FK)$.

\item \textit{TreeBased}: This is a post-hoc approach in which we first train a decision tree that includes $FK$. Internally, the tree induces a partitioning of 
...
the co-domain of $f$ is then only $[r]$ instead of $[l]$.
\end{packeditems}
