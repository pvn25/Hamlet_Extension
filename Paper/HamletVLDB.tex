...
Table~\ref{Table:RealTest1} presents the results.
...
package ``RWeka'' in R).
Surprisingly, as Table~\ref{Table:RealTest1} shows,  its accuracy is sometimes comparable to decision trees and RBF-SVMs! 
...
For all the high capacity classifiers, we notice average runtime speedup of 2x for \textit{NoJoin} over \textit{JoinAll} as shown in Figure \ref{Figure:runtime}. \textit{NoJoin} achieves the highest speedup of 3.6x and 6.2x on \textit{Movies} for decision tree and RBF-SVM respectively. And for ANN, \textit{LastFM} and Books dataset noticed the maximum speedup. The speedup for linear models are more significant. For example, \textit{movies} dataset yielded the highest speedup of 707x for naive bayes with backward feature selection and on \textit{LastFM} dataset the speedup is 20x for logistic regression with l1 regularizer.  
