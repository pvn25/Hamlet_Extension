...
with data systems (e.g.,~\cite{madlib,ore,bismarck,riot,longview}), how to scale ML 
algorithms (e.g.,~\cite{dimmwitted,graphlab,mllib,graphlab}), and how to use database 
ideas to support ML tasks (e.g.,~\cite{columbus,mlbase}).
However, little work has tackled the pains of \textit{sourcing} data for ML tasks in the first place,
especially, how fundamental data properties affect end-to-end data sourcing workflows for ML 
tasks in practice.
%
...
at multiple enterprise and Web companies, we learned that even this simple process of procuring 
tables for joins could be painful in practice because different tables are often 
``owned" by different teams within a company and have different access restrictions. This 
slows down the ML analytics lifecycle.
...
records, but crucially, do so without significantly affecting ML prediction accuracy. The 
theoretical basis for this dramatic capability is that a KFK join creates a \textit{functional dependency} 
(FD) between a \textit{foreign key feature} (present in the main table with the target for 
prediction) and the dependent \textit{foreign features} brought in by the join.\footnote{While 
KFKDs are not the same as FDs~\cite{avibook}, assuming features have ``closed" domains, 
they behave essentially as FDs in the output of the join~\cite{hamlet}.}

\vspace{1mm}
\noindent{\textbf{Example (based on~\cite{hamlet}).}}
Consider a common ML classification task in enterprise domains: predicting \textit{customer 
...
\texttt{Customers}.\texttt{Employer} turns out to be a foreign key feature that connects these tables. 
She joins both tables to bring in the foreign features (about employers) because she has a hunch that 
customers employed by rich corporations in coastal states might be less likely to churn. She then tries 
different ML classifiers, e.g., logistic regression or decision trees.

The theoretical analysis in~\cite{hamlet} established a fundamental dichotomy in the implication of a 
KFKD/FD for the bias-variance trade-off in ML, which explains the accuracy behavior of ML 
classifiers~\cite{hastie,shai}. Essentially, avoiding the join is unlikely to increase 
the bias but it might significantly increase the variance, since foreign key features often have much larger 
domains than any foreign feature. This means that avoiding a join could cause \textit{extra overfitting}.
However, the significance of this potential extra overfitting depends on the so-called \textit{tuple ratio} 
(ratio of the number of labeled customers to the number of employers), and it is mitigated when the 
tuple ratio goes above a certain threshold, i.e., when there are ``enough" training examples per foreign key value. 
For simple classifiers with linear VC dimensions, this threshold was about $20$. Many public real-world 
datasets satisfied this threshold, which meant this idea of ``avoiding joins safely," could be validated 
empirically. While initially controversial, this idea was eventually adopted by many data scientists, 
including some at Facebook, LogicBlox, and MakeMyTrip, since it helped reduce the burden of data 
sourcing and also reduced runtimes of ML tasks~\cite{perscomm}. However, the results 
in~\cite{hamlet} were restricted to linear classifiers. In fact, their VC dimension-based analysis suggested 
that the tuple ratio threshold for avoiding joins safely might be much higher for high-capacity non-linear classifiers.

\textit{In this paper, we perform a comprehensive empirical and simulation study to verify (or refute) 
the applicability of the idea of avoiding joins safely for three popular high capacity classifiers:
