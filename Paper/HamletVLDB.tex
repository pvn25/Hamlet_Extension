...
exploiting KFKDs present in the database \textit{schema}. That is, one can ignore a table 
without even looking at its contents, but crucially, do so without significantly affecting ML accuracy. 
...
real-world datasets with KFK joins from~\cite{hamlet} for these models. For the linear classifiers, the 
KFK joins could be avoided in only about half of the cases and that too with the tuple ratio threshold of $20$. 
But for these high capacity classifiers, we find that the joins can be 
...
To understand the above surprising behavior in depth, we conduct an in-depth Monte Carlo-style simulation study using a 
decision tree, which was the most robust to avoiding joins. We generate data for a two-table KFK join 
and embed various ``true'' distributions for the target. This includes a known ``worst-case'' scenario for avoiding joins with linear 
...
Understanding how fundamental properties of data sources affect ML behavior is one promising 
step in this direction. While the idea of avoiding joins safely has been adopted  in practice for 
...
and new research on theory and systems to simplify data sourcing for ML tasks.
